...
independent auditor (`mdcritique`) that loops back into it, a `mdgit` skill
that applies the project's version-control rules to any change, and a `mddocs`
...
ask the assistant to propagate: `/mdtechnical` ingests the changed docs, and the rest
...
(`mdfix` re-triggers `mdevaluation`; `mdcritique` dispatches to `mdtechnical`/`mdfix`).
...
| 3 | `mdevaluation` | `specs/evaluation.md` (the `EVAL-NNN` criteria) + the codebase | `specs/evaluation/report{NN}.md` | `mdfix` |
| 4 | `mdfix` | latest `specs/evaluation/report{NN}.md` + the specs it cites | code edits, then a fresh report | re-runs `mdevaluation` |
| 5 | `mdcritique` | all of `specs/` + the codebase | edits to specs; questions to the user | `mdtechnical` / `mdfix` |
| 6 | `mdgit` | `specs/git.md` + the working-tree diff | branch, commit, push/PR via `git`/`gh` | — (run after any step that changed files) |
| 7 | `mddocs` | `$ARGUMENTS` (a docs-change request); may read any file to document it accurately | **edits to `docs/` only** — touches nothing outside `docs/` | `mdtechnical` (to propagate documented intent into specs/code) |
...
> Standalone note: `modify` is **not** part of the forward chain either. It is the
> human-in-the-loop change loop: when you want to change something in the code **and**
> keep the docs in sync, `/modify <what to change>` applies the change in the code,
> verifies it (Quality Gates + before/after), then **asks you to confirm it is what you
> wanted**. Only once you confirm does it hand off to `/mddocs` (`EXECUTE_COMMAND: mddocs …`)
> so the documentation reflects the new behaviour. It writes code/tests/env-template,
> never runs git, and never auto-runs `/mddocs` without your confirmation.
...
```
        ...
        └───┤  independent auditor of ALL specs; dispatches resolution: │
            ...
            │   • product design ...... edit docs/, then /mdtechnical    │
            │   • architecture ........ /mdtechnical                     │
...
- **`fix → evaluation`** is the only built-in loop: after repairing findings,
  `mdfix` re-triggers `mdevaluation` to confirm the fixes hold. Iterate until the
  report is clean.
- **`mdcritique`** stands outside the chain. It reads everything, then *routes*
  each problem to the layer that **owns** it rather than patching the symptom —
  product-design changes go to `docs/` (then re-run `/mdtechnical`), `/mdtechnical` for
  architecture/criteria changes, `/mdfix` for code-vs-spec drift, direct edits for
...
/mdimplement                                           # → code
/mdevaluation                                          # → specs/evaluation/report01.md
/mdfix                                                 # repairs findings, re-evaluates
...
Run `/mdevaluation` (produces a numbered gap report) then `/mdfix` (closes the
gaps and re-audits). Repeat until `report{NN}.md` shows no FAIL/PARTIAL.

### Sanity-check the specs themselves
Run `/mdcritique` whenever the documents may have drifted apart (e.g. several
...
- **Design changes start in `docs/`.** No skill edits product intent into
  `specs/` directly; the human edits `docs/`, then `/mdtechnical` propagates. This is
...
- **`mdtechnical` front-loads permissions so the rest of the chain is unattended.**
  ...
  Gates, git policy, and tasks will run, so `/mdimplement` → `/mdevaluation` → `/mdfix`
...
- **Loop on `mdfix`/`mdevaluation`** until clean; reach for `mdcritique` when the
