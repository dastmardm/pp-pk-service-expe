...
1. A set of nodes is **parallel-eligible** when they have no dependency between them
   ...
   **and** their **write scopes are disjoint**. Determine both mechanically from
   ...
   Skills with overlapping scopes (e.g. `mdtechnical` and `mdevaluation`, which both write
   under `specs/`, or `mdcritique`/`mddocs`/`mdrevdocs`, which all write under `docs/`) are
   **not** eligible. Dispatch a parallel-eligible set
