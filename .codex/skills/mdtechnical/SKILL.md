...
artefacts) **plus** the harness permission allowlist (`.claude/settings.json`), and
...
2. **Audit ONLY `specs/`.** The critic may read **only files under `specs/`** — not
   `docs/`, not the code, not `.claude/`, not anything else. It checks the spec set
...
4. **Any gap → reflect into `./docs/` ONLY, then restart from scratch.** A gap in the
   ...
   - **Do not patch `specs/`.** Do not edit code, `.claude/settings.json`, or anything
