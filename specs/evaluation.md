...
| EVAL-006 | REQ-003, CONST-13, NFR-003 | Constitution | TERMite client is NOT imported at module load time in `enhance.py` (lazy import) | `src/oppp/stages/enhance.py`: TERMite import inside function body or conditional block, not at top-level | MAJOR |
...
| EVAL-019 | REQ-013 | Requirement | `execute_count` returns `ExecutionResult` with required field `ok` and optional fields `count_total`, `status`, `error`; `ok=true` when API returns a valid count response | `src/oppp/execute.py`: `execute_count` function; `src/oppp/models.py`: `ExecutionResult` with `ok` required and `count_total`, `status`, `error` optional; `tests/test_stages.py` or `test_pipeline.py`: mocked count test | MAJOR |
...
| EVAL-030 | REQ-022 | Requirement | `dag.py` exported stage list contains all eight labels: Stage -1, Stage 0, Stage 1, Stage 2A, Stage 3A, Stage 2B, Stage 3B, Stage 2C | `src/oppp/dag.py`: string constants or list with all eight stage names; `tests/test_dag.py`: assertion | MAJOR |
...
| EVAL-065 | CONTRACT-GROUNDING | Data contract | `src/oppp/models.py` exports `Grounding` with fields `matched`, optional `expanded_from`, and `confidence`; and `GroundingHit` with fields `name`, optional `id`, optional `parent_id`, optional `parent_name`, `score`, `match`, optional `count` | `src/oppp/models.py`: `Grounding` and `GroundingHit` class definitions with all listed fields | MAJOR |
| EVAL-066 | REQ-020 | Requirement | `oppp services` command exists and produces output listing at least the active PK service name and search URL | `src/oppp/cli.py`: `services` subcommand defined; `tests/test_cli.py`: `oppp services` invocation returns non-empty output | MINOR |
| EVAL-067 | CONST-9, REQ-011 | Constitution | Drug class label (e.g. "Kinase inhibitors") is emitted as a single class-label value resolved server-side, not expanded to individual drug members inline | `src/oppp/taxonomy/index.py`: drug class lookup returns class label without member expansion; `tests/test_taxonomy.py` or `test_stages.py`: drug class test case | MINOR |
| EVAL-068 | CONST-9, REQ-011 | Constitution | `sources.csv` hierarchy (document → FDA/EMA parent) is accessible via `lookup_sources` with `expand="parent"` | `src/oppp/taxonomy/index.py`: `lookup_sources` supports parent expansion; `tests/test_taxonomy.py`: sources parent expansion test | MINOR |
...
| REQ-009 | EVAL-013 |
| REQ-010 | EVAL-014 |
| REQ-011 | EVAL-015, EVAL-016, EVAL-067, EVAL-068 |
...
| CONST-9 (hierarchy reusable) | EVAL-015, EVAL-067, EVAL-068 |
...
- **LangGraph graph correctness** (`build_langgraph()`): the LangGraph wrapper is a convenience surface over the same fixed stages; its graph topology is not separately evaluated. W1.4.1 requires the function to exist and return a callable graph; behavioral correctness of the LangGraph topology is not evaluated.
