...
| `CONTRACT-SUBQUERY` | Stage-2A `MachineSubquery{field, operator, value\|pattern, boolean_group?, grounding?, dropped}`. `dropped=true` excludes invalid input closed-set filters from downstream assembly. |
...
| `CONTRACT-EXECUTION-COUNT` | `{ok, count_total?, status?, error?}` from `data.countTotal`. Used for evaluation and debugging. |
...
| `CONTRACT-SERVICE-CONFIG` | `ServiceConfig` and `FieldSpec` define field buckets (`closed`/`open`/`enum`/`boolean`), `EARLY_CONTRIBUTOR_THRESHOLD` (default 500), backing taxonomy paths, emitted API fields, facet allow-list, service invariants (`concomitants`, `tissueSpecific`, `metabolitesEnantiomers`), and search URL. |
