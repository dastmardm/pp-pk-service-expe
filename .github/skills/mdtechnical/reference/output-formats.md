...
derived from, so downstream skills (implement → evaluation) can trace lineage
upstream. Record:
...
## Harness permissions — `.sdd/settings.json`

```jsonc
// .sdd/settings.json  — illustrative; derive the real rules from the specs above.
{
  "permissions": {
    "allow": [
      "Edit",
...
Rules for writing `.sdd/settings.json`:
...
- **Merge, don't clobber.** If `.sdd/settings.json` already exists, add the
